...
class ScanFilters(BaseModel):
    min_market_cap: int = Field(default=100_000_000, ge=0)
    max_filing_delay_days: int = Field(default=14, ge=0, le=365)
    exclude_penny_stocks: bool = True
    lookback_days: int = Field(default=60, ge=1, le=180)
    cluster_window_days: int = Field(default=30, ge=1, le=90)
...
class ScanResult(BaseModel):
    ...
    peer_averages: dict[str, float | None]
